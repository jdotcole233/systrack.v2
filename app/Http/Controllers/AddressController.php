                ...
                // $name = DB::table('employees')->select('first_name','other_name','last_name','position')->where('emp_id', Auth::user()->emp_id)->where('delete_status','NOT DELETED')->first();
                 ...
                 if($size_of_email > 0){
                     ...
                     $cc_size = count($cc_email);
                       if($cc_size == 0){
                         $mail->to($emails[0])->send(new firmusMails($request->input('client_message'), $request->input('subject'),$request->input('client_name'),$request->input('reference_number'),$request->input('job_title') ));
                       }else if($cc_size > 0){
                         $mail->to($emails[0])->cc($cc_email)->send(new firmusMails($request->input('client_message'), $request->input('subject'),$request->input('client_name'),$request->input('reference_number'),$request->input('job_title') ));
                       }
                 }

                 return response()->json(['message'=>'message set successfully']);
            } else if($request->input('choice') === "Email"){
                 $name = DB::table('employees')->select('first_name','other_name','last_name','position')->where('emp_id', $request->input('employee_id'))->where('delete_status','NOT DELETED')->first();
                 $mail->to($request->input('contact_email'))->send(new firmusContactAddressEmail($request->input('client_message'), $request->input('subject'), $name));
