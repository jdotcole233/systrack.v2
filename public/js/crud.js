...
function add(id, key ,value) {
    ...
    //name="'+ value +'"
    if(id == "details")
        var details = '<div class="col-md-6"><div class="form-group row"><div class="col-md-10"><input placeholder="'+ key +'" value="'+ value +'" class="form-control col-md-10"  autocomplete ';
    else
        var details = '<div class="col-md-6"><div class="form-group row"><div class="col-md-10"><input placeholder="'+ key +'" value="'+ value +'" class="form-control col-md-10"  autocomplete ';
    key = key.toUpperCase().trim();
    if(key.indexOf("DATE") !==-1)
        details += 'type="date"></div><div class="col-md-2"><button onclick="addRemove(\''+ remove_id +'\')" style="border-radius: 100%; font-size: 12px;" class="btn-danger">X</button></div></div></div>';
    else if(key.indexOf("EMAIL") !==-1)
        details += 'type="email"></div><div class="col-md-2"><button onclick="addRemove(\''+ remove_id +'\')" style="border-radius: 100%;  font-size: 12px;" class="btn-danger">X</button></div></div></div>';
    else if(key.indexOf("ADDRESS") !==-1)
        details = '<div class="col-md-12"><div class="form-group"><textarea autocomplete placeholder="'+ key +'" class="form-control">'+ value +'</textarea><div class="col-md-2"><button onclick="addRemove(\''+ remove_id +'\')" style="border-radius: 100%;  font-size: 12px;" class="btn-danger">X</button></div></div></div>';
    else
        details += 'type="text"></div><div class="col-md-2"><button onclick="addRemove(\''+ remove_id +'\')" style="border-radius: 100%;  font-size: 12px;" class="btn-danger">X</button></div></div></div>';
...
function addRemove (id) {
    ...
    if(!isNaN(Number(id))) {
        alert(Number(id));
        $('#removed_tasks').append('<input class="removed_add" type="hidden" value="' + Number(id) + '"  />');
    }
}
    ...
    node.childNodes.forEach(function (node, index, listObj) {
        ...
        if ((node instanceof HTMLInputElement) || (node instanceof HTMLTextAreaElement)) {
            ...
            if (node.name.trim().length != 0)
                ...
                name = node.options[node.selectedIndex].value;
...
function reversePopulate (map , id, pop) {
    ...
    map.forEach(function(value, key) {
        if(key != '_token') {
            ...
            else if (document.getElementsByName(key)[0] instanceof HTMLInputElement || document.getElementsByName(key)[0] instanceof HTMLTextAreaElement)
                        ...
                        success : function (data) {
                            ...
                            if (data.message === 'success') {
                                ...
                                $.ajax({
                                    ...
                                    data : {
                                        message : 'Job' + data.data.job_name + 'has been edited',
                                        subject : 'Job Edited',
                                        check : 'position',
                                        condition : '<>',
                                        reciever : '1'
                                ...
                                $.ajax({
                                    ...
                                    url: '/admin-add_tasks',
                                    success: function (data) {
                                         swal({title: "success", text: "Update added Successfully", icon:"success"});
                      ...
                      if (willDelete) {
                                ...
                                $.ajax( {
                                    ...
                                    success : function (data) {
                                        $('#loading_progress').modal('hide');
                                        $('#con-close-modal').modal('hide');
                                        swal({title : "Firmus Job Type", text : "Job added Successfully", icon : 'success'});
                                        if (data.message === 'success') {
                                            $.ajaxSetup({
                                                headers: {
                                                    'X-CSRF-TOKEN' : $('meta[name="csrf_token"]').attr('content')
                                            ...
                                            $.ajax({
                                                ...
                                                url : '/sendNotification',
                                                data : {
                                                    message : 'Job' + data.data.job_name + 'has been added',
                                                    subject : 'Job Added',
                                                    check : 'position',
                                                    condition : '<>',
                                                    reciever : '1'
                                                ...
                                                error : function(data){
                                                $('#loading_progress').modal('hide');
                                                    console.log('Error sending notifaction for new added job');
                                                }
                                            });
                                              $('#loading_progress').modal('show');
                                            $('#job_task_id').val(data.job_id);
                                    ...
                                    error : function () {
                                   $('#loading_progress').modal('hide');
                                        swal({title:"FIRMUS JOBS", text:"Check for consistency and try again",icon:"warning"});
                                        // console.log("error");
                                    }
                                });
                            } else {
                     $('#loading_progress').modal('hide');
                    swal({title:"FIRMUS JOBS", text:"Job not created",icon:"success"});
                  }
                });
});
                        ...
                        $.ajax( {
                            ...
                            success : function (data) {
                                ...
                                $.ajax({
                                    ...
                                    url : '/sendNotification',
                                    data : {
                                        message : 'Job' + data.data.job_name + 'has been deleted',
                                        subject : 'Job Deleted',
                                        check : 'position',
                                        condition : '<>',
                                        reciever : '1'
...
$('#submit_job_request').on('click', function (e) {
    ...
    swal({
              ...
              if (willDelete) {
                    ...
                    $.ajax( {
                        ...
                        success : function (data) {
                            ...
                            var job_request_information = '<tr id="Job' + data.Data_back.job_id + ' " ><td>'
                                + data.Data_back.job_name + '</td><td>' + data.client_info.company_name  + '</td><td>'
                                + data.Data_back.status + '</td><td>' + data.Data_back.created_at + '</td><td>'
                                + data.Data_back.created_at + '</td></td>' ;

                            job_request_information += '<td><button class="btn btn-primary client_edit_info" value="  ' + data.Data_back.client_id
                                + ' ">Edit</button> <button class="btn btn-danger client_delete_info" value=" '
                                + data.Data_back.job_id +'">Delete</button></td></tr>';
                            ...
                            $.ajax({
                                ...
                                url : '/sendNotification',
                                data : {
                                    message : 'Job Request' + data.Data_back.reference_number + 'has been added',
                                    subject : 'Job Request',
                                    check : 'position',
                                    condition : '=',
            ...
            document.getElementById('jobsMenu').options[document.getElementById('jobsMenu').selectedIndex].value = JSON.stringify(info);
...
$('#edit_job_request').on('click', function (e) {
    ...
    // var info = JSON.parse();
    document.getElementById('jobsMenu').options[document.getElementById('jobsMenu').selectedIndex].value = $('#jobsMenu').val();
    swal({
    ...
    $.ajax( {
        method : 'post',
        data  : $('#job_request_form').serialize(),
        url  : '/manager-jobs_edit',
        success : function (data) {
            $('#loading_progress').modal('hide');
            swal({title : "Firmus Job Request", text : "Job Request Edited Successfully", icon : 'success'});
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN' : $('meta[name="csrf_token"]').attr('content')
                }
            ...
            $.ajax({
                ...
                url : '/sendNotification',
                data : {
                    message : 'Job Request' + data.Data_back.reference_number + 'has been updated',
                    subject : 'Job Request',
                    check : 'position',
                    condition : '=',
                    reciever : 'Finance Officer'
        ...
        error : function () {
            ...
            // console.log("error");
                ...
                $('#loading_progress').modal('hide');
        swal({title: "job Request", text:"Job Request details not updated", icon: "warning"});
    }
    });
});
        ...
        if (willDelete) {
            ...
            $.ajax( {
                ...
                success : function (data) {
                    ...
                    swal({title : "Firmus Job Request", text : "Job Request Deleted Successfully", icon : 'success'});
                    $.ajaxSetup({
                        headers: {
                            'X-CSRF-TOKEN' : $('meta[name="csrf_token"]').attr('content')
                        }
     ...
     $.ajax({
         ...
         success: function(data){
             ...
             $.each(data.employee_details, function (key, value) {
                 tr_info += '<tr id="sime'+key+'" class="' + data.job_assignment_details[key] + '" ><td >' + value +'</td><td><button type=\'button\' id="'+ key +'"  style="padding:0; width:28px; height:28px;" class=\'emp_remove btn-rounded btn-danger pull-right \'  onclick=\'emp_remove('+ key +')\'><i class=\'fa fa-minus-circle\'></i></button></td></tr>';
             });
...
$('#assignJobRequestSend').on('click', function (e) {
    ...
    removed_employee = removed_employee.substring(0, removed_employee.lastIndexOf(',')) + '}';
        ...
        success : function (data) {
            ...
            $.each(employees, function (key, value) {
                $.ajax({
                    ...
                    url : '/sendNotification',
                    data : {
                        message : 'Job request' + ref_no + 'has been assigned to you',
            ...
            $.each(employees_removed, function (key, value) {
                $.ajax({
                    ...
                    url : '/sendNotification',
                    data : {
                        message : 'You have been unassigned from Job Request' + ref_no,
...
$('.progress_button').on('click',function(){
  ...
  JSON.parse($('#job_request_contact_information').val()).forEach(function(value, key){
      $('#clientsEmail').append("<option value=" + JSON.parse(value.details)['EMAIL'] + ">"+ JSON.parse(value.details)['COMPANY NAME'] +"</option>");
  });
    ...
    details.forEach(function (value, key) {
        $('#assigned_employees').append('<tr><td>' + value.first_name + ' ' + value.last_name +  '</td></tr>');
    });
    ...
    tasks_details.forEach(function (value, key) {
        if($('#current_task_form').val().trim() == value.task_name.trim()) {
            $('#stages').append('<li class="task_info active" onmouseout="unshow()"  onmouseover="hover(this.id)" id=" ' + value.task_name +' "> </li>');
            bool = true;
        } else if(!bool) {
            $('#stages').append('<li  class="task_info cc active" onmouseout="unshow()" onmouseover="hover(this.id)" id=" ' + value.task_name +' ">'+'</li>');
        }else if(bool){
            $('#stages').append('<li  class="task_info" onmouseout="unshow()" onmouseover="hover(this.id)" id=" ' + value.task_name +' ">'+'</li>');
        }
    ...
    $(row_id).each(function (i, v) {
     ...
     job_assignment_id = job_assignment_id.substring(3,job_assignment_id.length);
...
$('#sendTasksUpdate').click(function(){
    ...
    swal({
                      ...
                      $('#loading_progress').modal('hide');
                        ...
                        $.ajaxSetup({
                         ...
                         $.ajax({
                            ...
                            url:'/send_email_firmus',
                            success: function(data){
                              swal({title:"Nofitication",text:"Automatic update sent to clients successfully",icon:"success"});
            ...
            swal({title:"Task Update",text:"Task not update",icon:"warning"});
        }
        });
});
    ...
    swal({
              ...
              if (willDelete) {
                    ...
                    $.ajax( {
                        ...
                        success : function (data) {
                           $('#loading_progress').modal('hide');
                            swal({title : "Firmus Meeting", text : "Meeting Created Successfully", icon : 'success'});
                            $.ajaxSetup({
                                headers: {
                                    'X-CSRF-TOKEN' : $('meta[name="csrf_token"]').attr('content')
                                }
                            ...
                            $.each(data.data, function (key, value) {
                                $.ajax({
                                    ...
                                    url : '/sendNotification',
                                    data : {
                                        message : 'Meeting about' + data.meeting.purpose + ' at' + data.meeting.date + ' ' + data.meeting.meeting_start_time + ' in ' + data.meeting.meeting_venue,
                        ...
                        error : function () {
                             $('#loading_progress').modal('hide');
                            swal({title : "Something Went Wrong", text : "Check all fields to make sure the right values are in there ", icon : 'warning'});
                        }
                    });
            }else{
                 $('#loading_progress').modal('hide');
...
function emp_remove(id) {
    ...
    if(class_job_assingment_id !== "") {
        $('#removed_important').append('<input id="' + id + '" class="job_assignment_id" type="hidden" value="' + class_job_assingment_id + '"  />');
        // console.log('done');
    }
...
$('.meeting_table').on('click','.view_update_btn',function(){
    ...
    $.ajax({
        ...
        $.each(data.m_b_p,function(i,v){
            td_on += '<tr id="sime' + v.meeting_attends_id + '" class="remove_them_moimoi"><td>'+ v.attendant_name + '</td><td>';
              td_on += '<button type="button" id="' + v.meeting_attends_id + '" style="padding:0; width:28px; height:28px;" class="emp_remove btn-rounded btn-danger pull-right"';
               td_on += 'onclick="emp_remove(this.id)"><i class="fa fa-minus-circle"></i> </button></td></tr>';
        });
...
$('#update_btn_minutes').click(function(){
    ...
    $('#invited_employees tr[class="remove_them_moimoi"]').each(function (i, v) {
        var string = $(this).attr('id');
        var name_attendee = $(this).attr('name');
        try{
            var id = string.substring(5, string.length);
    ...
    old_employee = old_employee.substring(0, old_employee.lastIndexOf(',')) + '}';
 ...
 swal({
              ...
              if (willDelete) {
                    ...
                    $.ajax({
                        ...
                        success : function(data){
                           $('#loading_progress').modal('hide');
                        swal({title : "Firmus Meeting Updates", text : "Meeting Updated Successfully", icon : 'success'});
                        $.ajaxSetup({
                            headers: {
                                'X-CSRF-TOKEN' : $('meta[name="csrf_token"]').attr('content')
                            }
                        ...
                        if(jsonData !== "{") {
                            $.each(JSON.parse(jsonData), function (key, value) {
                                $.ajax({
                                    ...
                                    url : '/sendNotification',
                                    data : {
                                        message : 'Meeting about' + $('#meeting_purpose_back').val() + ' at' + $('#meeting_date_back').val() + ' ' + $('#meeting_time_back').val() + ' in ' + $('#meeting_venue_back').val() + 'has been postponed',
                                        subject : 'Meeting '+ $('#meeting_title_back').val() + 'has been postponed',
                                        check : 'emp_id',
                                        condition : '=',
                                        reciever : key
                                    },
                        ...
                        if(old_employee !== '{') {
                            $.each(JSON.parse(old_employee), function (key, value) {
                                $.ajax({
                                    ...
                                    url : '/sendNotification',
                                    data : {
                                        message : 'Meeting about' + $('#meeting_purpose_back').val() + ' at' + $('#meeting_date_back').val() + ' ' + $('#meeting_time_back').val() + ' in ' + $('#meeting_venue_back').val() + 'has been updated',
                                        subject : 'Meeting '+ $('#meeting_title_back').val() + 'has been updated',
                                        check : 'emp_id',
                                        condition : '=',
                                        reciever : key
                                    },
                        ...
                        if(removed_employees !== '{') {
                            $.each(JSON.parse(removed_employees), function (key, value) {
                                $.ajax({
                                    method: 'POST',
                                    dataType: 'json',
                                    url : '/sendNotification',
                                    data : {
                                        message : 'You have been removed from the meeting about' + $('#meeting_purpose_back').val() + ' at' + $('#meeting_date_back').val() + ' ' + $('#meeting_time_back').val() + ' in ' + $('#meeting_venue_back').val() + 'has been updated',
                                        subject : 'Meeting'+ $('#meeting_title_back').val() + 'has been updated',
                                        check : 'emp_id',
                                        condition : '=',
                                        reciever : key
...
$('.cancel_meeting').click(function(e){
    swal({
            ...
            $.ajax({
                ...
                success : function(data){
                $('#loading_progress').modal('hide');
                swal({title : "Firmus Meeting Updates", text : "Meeting Cancelled", icon : 'success'});
                $.ajaxSetup({
                    headers: {
                        'X-CSRF-TOKEN' : $('meta[name="csrf_token"]').attr('content')
                    }
                ...
                $.each(data.data, function (key, value) {
                    $.ajax({
                        ...
                        url : '/sendNotification',
                        data : {
                            message : 'Meeting about' + data.meeting.purpose + ' at' + data.meeting.date + ' ' + data.meeting.meeting_start_time + ' in ' + data.meeting.meeting_venue + 'has been cancelled',
                            subject : 'Meeting '+ data.meeting.title + 'Cancelled',
                            check : 'emp_id',
                            condition : '=',
                            reciever : value.emp_id
                        },
...
swal({
              ...
              if (willDelete) {
                  ...
                  if(amount_paid > amount_deficit && payment_status != 'REFUND') {
                      swal({title : "Firmus Job Payment", text : "Check payment details, You Cannot OverPay", icon : 'warning'});
                  } else if(amount_paid >= 0) {
                        if (payment_status == 'REFUND') {
                            $('[name=job_cost]').val(job_cost - amount_paid);
                            // $('[name=been_payed]').val(amount_paid + been_payed);
                            $('[name=amount_deficit]').val(amount_deficit - amount_paid);
                        } else if(amount_paid <= amount_deficit) {
                            $('[name=been_payed]').val(amount_paid + been_payed);
                            $('[name=amount_deficit]').val(amount_deficit - amount_paid);
                        } else {
                            setTimeout(function (){$('#loading_progress').modal('hide')}, 2000);
                            swal({title : "Firmus Job Payment", text : "Check payment details", icon : 'warning'});
                            return;
                        }
                        $('#loading_progress').modal('show');
                        $.ajax( {
                            ...
                            success : function (data) {
                                $('#loading_progress').modal('hide');
                                swal({title : "Firmus Job Payment", text : "Payment Successful", icon : 'success'});
              ...
              $('#loading_progress').modal('hide');
            swal({title: "Firmus Job Payment", text:"Not Paid", icon: "warning"});
        }
        });
});
                 ...
                 $.ajaxSetup({
                    ...
                    $.ajax({
                        ...
                        success : function(data){
                            ...
                            swal({title:"Success",text: "Successfully Updated", icon:"success"});
                            $.ajax({
                                method: 'POST',
                                dataType: 'json',
                                url : '/sendNotification',
                                data : {
                                    message : 'Contact ' + data.data.contact_name + 'has been updated.',
                    ...
                    $.ajax({
                        ...
                        success : function(data){
                            $.ajax({
                                ...
                                url : '/sendNotification',
                                data : {
                                    message : 'Contact ' + data.data.contact_name + 'has been deleted',
...
$('#save_con').on('click',function(){
     swal({
            ...
            swal({title: "Contact", text:"Contact not added", icon: "warning"});
        }
        });
});
...
$('.save_changes').on('click',function(){
     swal({
                ...
                $.ajax({
                    ...
                    success: function(data){
                     $('#loading_progress').modal('hide');
                        $('#cont_forms').trigger('reset');
                        $('#con-close-modal').modal('hide');
                        var  login_details = 'USERNAME : '+ data.employe_log_details.username +'\n';
            ...
            swal({title: "Employee", text:"Employee not added", icon: "warning"});
        }
        });
});
...
function getStats () {
    ...
    if(window.location.pathname == '/finance-home' || window.location.pathname == '/finance/home/partner-finance') {
        getFinanceStats();
    } else if (window.location.pathname == '/manager-home/manager' || window.location.pathname == '/manager/home/partner') {
        getManagerStats();
    } else if (window.location.pathname == '/admin-home' || window.location.pathname == '/admin/home/partner-admin') {
        getAdminStats();
...
if(window.location.pathname != 'track' || window.location.pathname != 'tracked-result'){
...
$('#generate_report').click(function(){
    ...
    if (job_selected == "SJ" || from_date == "" || to_date == ""){
      ...
      swal({title: "Error", text: "Check Mandatory field", icon:"warning"});
      return;
    }else {
    ...
    $.ajax({
      ...
      success: function(data){
        ...
        if (Object.keys(data.data).length == 0){
          swal({title: "Query result", text: "No queries found", icon:"success"});
        }
...
function progress_view(job_request_id) {
    document.getElementById('job_request_id').value = document.getElementById('job_request'+job_request_id).textContent;
    document.getElementById('tasks_details').value = document.getElementById('tasks'+job_request_id).textContent;
    document.getElementById('current_task_form').value = document.getElementById('current_task'+job_request_id).textContent;
    document.getElementById('current_task_id').value = document.getElementById('current_task_id'+job_request_id).textContent;
    document.getElementById('display_task').innerHTML = document.getElementById('current_task'+job_request_id).textContent;
    document.getElementById('current_task_form_job_request_id').value = document.getElementById('job_request_id_id'+job_request_id).textContent;
    document.getElementById('company_email').value = document.getElementById('company_email'+job_request_id).textContent;
}
